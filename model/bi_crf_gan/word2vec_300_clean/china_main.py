...
import random
from bert_gan import Generator_BiLSTM_CRF
import numpy as np
...
# def run_one_batch(sess, words,labels,tags, dev,epoch,gen ,num_batches,batch,label):
...
#  print(loss_train)
# print('333333333333333333333,labeled training of discriminator finised!,ppppppppppppppppppppppppppppppppp')
...
def main():
    ...
    #    sess.run(init)
    #   sess.run(tf.local_variables_initializer())
    #    generator = Generator_BiLSTM_CRF(0.5, params,1)
    #   discriminator = Discriminator(sequence_length=20, num_classes=2,filter_sizes=dis_filter_sizes, num_filters=dis_num_filters, l2_reg_lambda=dis_l2_reg_lambda,embedding_size =200)
    ...
    if args.mode == 'train':
        ...
        #          for batch in range(num_batches_1):
        #              train(sess,train = batches_labeled_for_dis[batch], dev=test_data,epoch = epoch_total,gen = generator,num_batches =100 ,batch = batch,label =2)
        #              train(sess, train= batches_unlabeled_for_dis[batch], dev=test_data, epoch=epoch_total, gen=generator,num_batches =100 ,batch = batch, label=3)
        ...
        for epoch_total in range(30):
            ...
            #    train(sess, train=batches_labeled[batch], dev=test_data, epoch=epoch_total, gen=generator,
            #          num_batches=num_batches, batch=batch, label=0)
            ...
            if epoch_total > 5:
                ...
                #                    train(sess,train = batches_labeled_for_dis[batch], dev=test_data,epoch = epoch_total,gen = generator,num_batches =num_batches_1 ,batch = batch,label =2)
                #                    train(sess, train= batches_unlabeled_for_dis[batch], dev=test_data, epoch=epoch_total, gen=generator,num_batches =num_batches_1 ,batch = batch, label=3)
                ...
                #        for batch in range(num_batches_unlabel):
                #            if batch < num_batches_unlabel:
                #                train(sess, train=batches_unlabeled[batch], dev=test_data, epoch=epoch_total, gen=generator,
                #                      num_batches=num_batches_unlabel, batch=batch, label=1)
                #            if batch >= num_batches_unlabel:
                #                batch_tra = random.randint(1, 80)
                #                train(sess, train=batches_unlabeled[batch_tra], dev=test_data, epoch=epoch_total, gen=generator,
                #                     num_batches=num_batches_unlabel, batch=batch, label=1)
            ...
            #  if epoch_total >6 and epoch_total % 2 == 1 and index % 2 == 1:

            #        for batch in range(num_batches_1):
            #           train(sess,train = batches_labeled_for_dis[batch], dev=test_data,epoch = epoch_total,gen = generator,num_batches =num_batches_1 ,batch = batch,label =2)
            #           train(sess, train= batches_unlabeled_for_dis[batch], dev=test_data, epoch=epoch_total, gen=generator,num_batches =num_batches_1 ,batch = batch, label=3)
            #       print('the whole dis phaseII finised')
            #       index += 1
            #       for it in range(5):
            #           for batch in range(num_batches_unlabel):
            #               if batch < num_batches_unlabel:
            ...
            #                   train(sess, train=batches_unlabeled[batch_tra], dev=test_data, epoch=epoch_total, gen=generator,
            #                        num_batches=num_batches_unlabel, batch=batch, label=1)
            #       dev3 = batch_yield(test_data, batch_size, shuffle=True)

            #      medi_lis_from_adversarial_training2 = get_metrics(sess, generator, dev3, test_size, batch_size, flag=0)

            #     for ele in medi_lis_from_adversarial_training2:
            #         print(ele)
            #                print('the accuracy after adversarial training of generator finished!!!!!!!!!!!!!!!!!!1! ')

            print('epoch {} finished!'.format(epoch_total + 1))
