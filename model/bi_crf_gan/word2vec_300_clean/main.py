...
from data import batch_yield, read_corpus,pad_sequences, batch_yield_for_unla_da,read_corpus_unlabel, batch_yield_for_discri, batch_yield_for_discri_unlabeled
...
#    if args.mode == 'train':
    ...
    if args.mode == 'train':
        ...
        for epoch_total in range(20):
            ...
            for i, (words,labels) in enumerate(batches_labeled):
                run_one_epoch(sess, words,labels,tags=[],dev =test_data, epoch = epoch_total, gen=generator,num_batches=num_batches, batch=i,label=0,it = 0, iteration =0,saver =saver)
      ...
      #                           num_batches=num_batches_unlabel, batch=batch, label=1)
           ...
           #     batches_labeled_for_dis = batches_labeled_for_dis[0: len(batches_labeled_for_dis)-5]
                ...
                for (ele, ele2) in zip(enumerate(batches_labeled_for_dis), enumerate(batches_unlabeled_for_dis)):
                    index += 1
                    run_one_epoch(sess, ele[1][0], ele[1][1], ele[1][2], dev=test_data, epoch=epoch_total, gen=generator,
                                  num_batches=batch_dis_for_label, batch=index, label=2,it = 0,iteration = 0,saver = saver)
                    run_one_epoch(sess, ele2[1][0], ele2[1][1], ele2[1][2], dev=test_data, epoch=epoch_total, gen=generator,
                                  num_batches=batch_dis_for_unlabel, batch=index, label=3,it = 0,iteration = 0, saver = saver)
                index = 0
                ...
                for it in range(5):
                    for i, (words, labels,tags) in enumerate(batches_unlabeled):
                        run_one_epoch(sess, words, labels, tags=tags, dev=test_data, epoch=epoch_total, gen=generator,
                                      num_batches=num_batches_unlabel, batch=i, label=1,it = it,iteration = i,saver = saver)
  ...
  #          print(list(sub_dev))
        ...
        # print(ckpt_file)
        generator = Generator_BiLSTM_CRF(0.5, batch_size, params, filter_sizes, num_filters, 0.75, length, is_training=False)
        generator.build_graph()
